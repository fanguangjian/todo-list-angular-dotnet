...
public class TagsControllerTests : IClassFixture<TodoApiFactory>
{
    ...
    public async Task GetAll_ReturnsEmptyList_WhenNoTags()
    {
        ...
        var response = await _client.GetAsync($"/api/users/{user.Id}/tags");
    ...
    {
        var user = await CreateUser();
        var dto = new CreateTagDto("Work", "#FF5733");
        var response = await _client.PostAsJsonAsync($"/api/users/{user.Id}/tags", dto);
    ...
    public async Task Delete_RemovesTag()
    {
        ...
        var tags = await _client.GetFromJsonAsync<List<TagDto>>($"/api/users/{user.Id}/tags");
